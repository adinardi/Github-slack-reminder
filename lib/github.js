  ...
  getAllPending: function(callback){
    ...
    self.getAllRepos(function(err, repos){
      ...
      async.each(repos, function(repo, cb){
        self.getPullRequests(repo, function(err, pullReqs){
          ...
          pullReqs.map(function(a){
            a.pendingReviews.map(function(b){
              ...
              // Put the inverse
              var key = repo.id + "-" + a.id;
              if(pullReqUsersMapping[key]){
                pullReqUsersMapping[key]['pendingUsers'].push(b);
              }else{
                pullReqUsersMapping[key] = {
                  pullRequest: a,
                  pendingUsers: [b]
                };
              }
        ...
        return callback(err, {
          pendingUsersRequests: pendingUsersRequests,
          pendingPullRequestsUsers: pullReqUsersMapping
        });
      })
    })
  },
  ...
  getPendingRequestedReviewers: function(repo, pullRequest, callback){
    ...
    this.getReviews(repo, pullRequest.number, function(err, reviews){
      ...
      self.callGithubApi(url, true, function(err, requested_reviewers){
        ...
        // Get only the requested user reviewers.
        requested_reviewers = requested_reviewers.users;
        ...
        reviews.map(function(a){
          ...
          if(requested_reviewers.filter(function(b){
            return b.login == a.user.login
          }).length == 0){
